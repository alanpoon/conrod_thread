...
pub fn set_ui(ref mut ui: conrod::UiCell,
          ids: &Ids,
          demo_text: &mut String,history:&mut VecDeque<chatview::Message>,styles:Static_Style) {
    widget::Canvas::new().color(color::LIGHT_BLUE).set(ids.master, ui);
    // Instantiate the `Image` at its full size in the middle of the window.
    chatview::ChatView::new(history, demo_text,styles).middle_of(ids.master).set(ids.chatview, ui);
}
pub fn update_state(rust_logo:conrod::image::Id,conrod_msg:Conrod_Message,demo_text:&mut String,history:&mut VecDeque<chatview::Message>){
        if let Conrod_Message::Websocket(j) = conrod_msg.clone() {
        if let websocket::OwnedMessage::Text(z) = websocket::OwnedMessage::from(j){
        if let Ok(g_w_app::ReceivedMsg{type_name,tables,players,request,reason,optional,location,sender,message}) = g_w_app::deserialize_receive(&z){
            ...
            if type_name=="chat"{
                if let (Some(Some(_location)),Some(Some(_sender)),Some(Some(_message))) = (location,sender,message){
                    if _location=="lobby"{
