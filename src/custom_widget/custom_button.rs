...
use dyapplication;
use custom_widget::chatview_ownedmsg::Message;
use std;
...
pub struct CustomButton<'a> {
  ...
  pub  message: &'a Message,
    ...
    /// user input.
    enabled: bool,
   pub static_style: &'a dyapplication::Static_Style,
}
...
/// Represents the unique, cached state for our CustomButton widget.
pub struct State {
    ids: Ids,
}
...
impl<'a> CustomButton<'a> {
    ...
    pub fn new(message: &'a Message, static_style: &'a dyapplication::Static_Style) -> Self {
        CustomButton {
            message: message,
...
impl<'a> Widget for CustomButton<'a> {
    ...
    fn update(self, args: widget::UpdateArgs<Self>)->std::option::Option<()> {
        ...
        let mut c = 0.0;
        let (x, y, w, mut h) = rect.x_y_w_h();
        let dyapplication::RGB(rr, rg, rb, ra) = static_style.rect.0;
        widget::Rectangle::outline([static_style.rect.1, static_style.rect.2])
                .align_left_of(id)
                .color(color::Color::Rgba(rr,rg,rb,ra))
                .set(state.ids.rect,ui);

        widget::Image::new(self.message.image_id)
